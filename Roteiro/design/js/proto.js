...
(function () {
  ...
  SCR.splash = () => ({ inv: true, raw: '<div class="splash">' + U.logo(84, '#fff', '#fff') + '<div class="wm">roteiro</div><div class="tl">Lembre no lugar certo.</div><div class="ver">v1.0</div></div>' });
  ...
  SCR.ob = (s) => {
    const T = [
      ...
      [A.obArt2, 'Chegou no lugar certo? A gente lembra você.', 'Ao chegar ou sair de um lugar, o Roteiro mostra só o que importa ali. O resto espera.'],
  ...
  SCR.perms = () => {
    ...
    let b = U.top({ left: back_(), title: 'Permissões' }) + '<div class="h2">O que o Roteiro consegue perceber</div><div class="sub">Cada permissão libera um tipo de lembrete. Nada sai do seu aparelho.</div>';
    b += S.locDenied
    ...
    b += card('<div class="set" style="padding:0;border:0">' + I('battery') + '<div class="set-b">Economia de bateria<small>' + (S.battFixed ? 'Roteiro liberado' : 'Pode atrasar avisos em até 15 min') + '</small></div>' + (S.battFixed ? '<span class="st ok">Ok</span>' : btnA('Resolver', 's', 'battfix', null, 'sm')) + '</div>');
  ...
  function notifHTML(n, heads, anim) {
    return '<div class="nt-card' + (heads ? ' heads' : '') + (anim ? ' a-down' : '') + '" data-act="nopen"><div class="nt-hd"><span class="app-i">' + U.logo(14, '#fff', '#fff') + '</span>Roteiro' + (n.sub ? ' · ' + esc(n.sub) : '') + '<span class="tm">agora</span></div>' +
  ...
  function renderSim() {
    ...
    simEl.innerHTML =
      ...
      '<div class="sim-box"><h2>Caminho sugerido</h2><p>Um passeio de 2 minutos pelas partes mais importantes.</p><ol class="tour" style="margin-top:14px">' +
