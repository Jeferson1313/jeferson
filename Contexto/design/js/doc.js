/* =========================================================
   Contexto — monta a documentação a partir dos componentes
   ========================================================= */
(function () {
  ...
  $('ds-logo').innerHTML = '<div style="display:flex;gap:28px;align-items:center;flex-wrap:wrap">' +
    '<div style="display:flex;align-items:center;gap:12px">' + U.logo(48) + '<span class="wm" style="font-size:36px;color:var(--a-ink)">contexto</span></div>' +
  ...
  function setMode(m) {
    ...
    try { localStorage.setItem('contexto-mode', m); } catch (e) { /* sem armazenamento */ }
  }
  btns.forEach((b) => b.addEventListener('click', () => setMode(b.dataset.m)));
  let saved = 'final';
  try { saved = localStorage.getItem('contexto-mode') || 'final'; } catch (e) { /* sem armazenamento */ }
  setMode(saved);
})();
